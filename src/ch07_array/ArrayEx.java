...
public class ArrayEx {
	...
	public static void main(String[] args) {
		...
		String [] students = {"강지석","김도래","김병준","김서영","김소연",""
				+ "김원진","박정단","박종욱","송은호","신미정","이예성","이예은","정지나","최보석","한수정"};
	
		//학생들의 이름에 출석부 ㅂ순번을 붙여주세요 1.강지석.2.김도래..
	for(int i=0; i< students.length; i++) {
		System.out.println(students[i]);
	}
	for(int i=0; i< students.length; i++) {
		System.out.println((i+1) + "." + students[i]); 	///왜 +하면 숫자가 생기는??
	}
	
	//같은 성 찾기 메소드를 작성해주세요
	//input: String , String[]
	//output: 없음
	//호출:serachNm("이",students);
