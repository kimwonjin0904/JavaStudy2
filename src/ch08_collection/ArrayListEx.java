...
public class ArrayListEx {
	...
	public static void main(String[] args) {
		...
		while(true){
			System.out.println("탑100입니다. 가수 or 제목 어떤걸로 알려드릴깝쇼(종료q)");
			System.out.print(">>>");
			String select = scan.nextLine();
			if(select.equalsIgnoreCase("q")){
				System.out.println("탑100 종료===="); //equalsIgnoreCase 대소문자 상관없이 같은지
				break;
			}
			
			System.out.println("키워드 입력해주세요");
			System.out.print(">>>");
			String keyword = scan.nextLine();
			//1.메소드 사용하여 검색결과 가져오기
			ArrayList<String> result = searchSong(select, keyword, playList);
			System.out.println(result);
			//2.없으면 결과가 없습니다. 있다면 총 몇건인지출력후 순서대로 출력
			//없으면 (isEmpty()로 체크 가능)
			if(result.isEmpty()) {						
				System.out.println("아쉽지만 검색결과가 없습니다.");
			}else {
				//건수는 (size())
				System.out.println("검색결과는 총:" + result.size()+ "건");
	...
	public static ArrayList<String> searchSong(String option, 		///ArrayList<String> 스트링 붙이는 이유??
							String keyword, ArrayList<String>arr){
		//전달받은 option을 기준으로 keyword포함된 데이터를 찾아서 리턴
		ArrayList<String> result = new ArrayList<>();
		for(int i =0; i<arr.size(); i++) {
			//각 요소에서 :을 기준으로 0:제몽,1:가수
			int no = i +1;			//순서 넣는 거
			String [] temp = arr.get(i).split(":");		///split,temp 역할?
			if(option.equals("가수")) {						///equals 역할??
