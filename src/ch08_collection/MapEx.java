...
public class MapEx {
	...
	public static void main(String[] args) {
		...
		//가수정보만 출력(중복 x)
		HashSet<String> uniqeSingers = new HashSet<>();		//set사용
		for(int i = 0; i < playList.size(); i++) {
			String [] album = playList.get(i).split(":");		//split
			String singer = album[1];		//가수명
			uniqeSingers.add(singer);		//출력
		}
		System.out.println(uniqeSingers);
		System.out.println(uniqeSingers.size());
		
		//가수 이름을 오름차순으로 정렬 set to list
		ArrayList<String> uniqeList = new ArrayList<>(uniqeSingers);
		...
		for(String songEntry: playList) {
			...
			if(artistSongMap.containsKey(singer)) {
				//있다면 기존에 리스트 추가.
				ArrayList<String> songs = artistSongMap.get(singer);		//get
				artistSongMap.put(singer, songs);	//map수정
			}else {
				//없다면 새로운 리스트를 만들어서 추가
				ArrayList<String> songs = new ArrayList<>();
				songs.add(songTitle);			//1번째에 추가
				artistSongMap.put(singer, songs);	//map추가 ,배열형태
			}
		}
		System.out.println(artistSongMap);			//이창섭=[천상연] 이창섭이 키 천상연이 value
		...
		//해당가수 노래 출력
		HashSet<String> uniqeSings = new HashSet<>();		
		for(int i = 0; i < playList.size(); i++) {
			String [] album = playList.get(i).split(":");		
			String sings = album[1];		
			uniqeSings.add(sings);	
